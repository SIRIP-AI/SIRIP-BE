...
const currentPlan = { id: '10', version: 1, reason: 'Current', deadline, steps: [{ sequence: 1, ...step, status: 'COMPLETED' as const, completedAt: '2026-08-20T10:00:00.000Z', coldStorageId: null, vehicleId: null, destinationId: null, notes: null }] };
...
function view(status: PlanView['status']): PlanView {
  return { id: '10', version: 1, status, previousPlanId: '9', reason: 'Current', deadline, createdAt: context.now, approvedAt: null, completedAt: null, batches: [{ id: '7', code: 'B-7' }], trigger: null, steps: [] };
}
...
test('revising a proposal keeps its scope and requests transactional replacement', async () => {
  ...
  const service = new PlanService(repository, async (request) => {
    ...
    return { result: { status: 'FEASIBLE', ...proposal }, context };
  }, () => []);
  ...
  assert.equal(saved?.[3], deadline);
  assert.equal(saved?.[4], currentPlan);
  assert.deepEqual(saved?.[5], { replaceProposalId: 10n });
});
...
test('does not persist a workflow result that fails deterministic validation', async () => {
  ...
  const service = new PlanService(repository, async () => ({ result: { status: 'FEASIBLE', reason: 'Invalid', steps: [step] }, context }), () => ['Invalid']);

  await assert.rejects(() => service.generateProposal(1n, [7n], 3n, deadline), (error) => error instanceof RequestError && error.status === 502);
  assert.equal(saved, false);
});

test('does not persist an infeasible workflow result', async () => {
  let saved = false;
  const repository = { saveProposal: async () => { saved = true; return view('PROPOSED'); } } as unknown as PlanRepositoryPort;
  const service = new PlanService(repository, async () => ({ result: { status: 'INFEASIBLE', reason: 'No route reaches the destination in time.' }, context }), () => []);
