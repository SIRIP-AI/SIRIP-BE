...
Plan generation and natural-language revision run through LangGraph. Graph nodes load context, derive concrete UTC deadlines, feasible resources, resource flexibility, and urgency, generate deterministically validated candidates, and invoke the LangChain chat model to select the preferred candidate. Prisma remains authoritative for proposal persistence, human approval, completed-step history, and concurrency; graph execution does not save or activate plans. Approval does not rerun planning validation. Free-text vehicle restrictions remain advisory.
