...
test('accepts and validates an ESP32 telemetry reading', async (context) => {
  const ingest = context.mock.fn(async () => undefined);
  const app = express();
  app.use(express.json());
  app.use('/telemetry', createTelemetryRouter({ ingest }));
  ...
  try {
    const body = { sensorId: ' S-001 ', deviceUid: ' ESP32-001 ', temperature: 2.8, sequenceNumber: 1 };
    const response = await fetch(url, { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(body) });
    assert.equal(response.status, 200);
    assert.deepEqual(await response.json(), { acknowledged: true, sensorId: 'S-001', sequenceNumber: 1 });
    assert.deepEqual(ingest.mock.calls[0]?.arguments, [{ sensorId: 'S-001', deviceUid: 'ESP32-001', temperature: 2.8, sequenceNumber: 1 }]);

    for (const invalidBody of [
      { deviceUid: 'ESP32-001', temperature: 2.8, sequenceNumber: 1 },
      { sensorId: 'S-001', temperature: 2.8, sequenceNumber: 1 },
      { sensorId: 'S-001', deviceUid: 'ESP32-001', temperature: '2.8', sequenceNumber: 1 },
      { sensorId: 'S-001', deviceUid: 'ESP32-001', temperature: 2.8, sequenceNumber: -1 },
    ]) {
