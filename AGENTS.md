...
Plan generation and natural-language revision run through LangGraph. Graph nodes load context, derive concrete UTC deadlines, feasible resources, resource flexibility, and urgency, invoke the LangChain chat model with a structured response schema, parse `PROPOSAL` or `NO_VALID_PROPOSAL_FOUND`, refresh and rederive context, then run hard validation and deterministic quality policy. Quality policy blocks only demonstrably avoidable scarce-resource misuse, urgency inversion, and storage whose removal passes hard validation. Up to two shared repair attempts receive the rejected response and coded errors. Prisma remains authoritative for proposal persistence, approval, completed-step history, and concurrency; graph execution does not save or activate plans. The same combined evaluation runs before saving and again transactionally before approval. Free-text vehicle restrictions remain advisory.
