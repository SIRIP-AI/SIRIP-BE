...
function id(value: unknown, label: string) { try { if (typeof value !== 'string' && (typeof value !== 'number' || !Number.isSafeInteger(value))) throw new Error(); const parsed = BigInt(value); if (parsed <= 0n) throw new Error(); return parsed; } catch { throw new RequestError(`${label} harus berupa bilangan bulat positif`, 400); } }
function input(body: unknown): BatchInput { const value = object(body); if ('status' in value) throw new RequestError('status tidak boleh diberikan untuk batch', 400); if (typeof value.weightKg !== 'number' || !Number.isFinite(value.weightKg) || value.weightKg <= 0) throw new RequestError('weightKg harus lebih besar dari nol', 400); if (typeof value.receivedAt !== 'string' || !isoDateTime.test(value.receivedAt) || !Number.isFinite(Date.parse(value.receivedAt))) throw new RequestError('receivedAt harus berupa datetime ISO yang valid', 400); return { code: text(value, 'code'), fishingTripId: id(value.fishingTripId, 'fishingTripId'), weightKg: value.weightKg, grade: text(value, 'grade'), receivedAt: new Date(value.receivedAt).toISOString() }; }
function filter(value: unknown): BatchFilter | undefined { if (value === undefined) return undefined; if (typeof value !== 'string' || !batchFilters.includes(value as BatchFilter)) throw new RequestError(`filter harus salah satu dari: ${batchFilters.join(', ')}`, 400); return value as BatchFilter; }
