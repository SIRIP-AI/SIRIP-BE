...
export function createBatchesRouter(repository: BatchRepository) {
  const router = Router();
  router.get('/', async (request, response) => response.json(await repository.list(filter(request.query.filter))));
  router.post('/', async (request, response) => response.status(201).json(await repository.create(input(request.body))));
  router.put('/:id', async (request, response) => response.json(await repository.update(id(request.params.id, 'Batch ID'), input(request.body))));
  router.delete('/:id', async (request, response) => { await repository.delete(id(request.params.id, 'Batch ID')); response.sendStatus(204); });
  return router;
}
