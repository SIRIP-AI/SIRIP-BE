...
Plan generation and natural-language revision run through LangGraph. Graph nodes load context with concrete UTC resource windows and deterministic resource/window hints, invoke the LangChain chat model with a structured response schema, parse `PROPOSAL` or `NO_VALID_PROPOSAL_FOUND`, refresh context, and route up to two deterministic validation repairs. Repair prompts include the rejected response and actionable validator errors. Prisma remains authoritative for proposal persistence, approval, completed-step history, and concurrency; graph execution does not save or activate plans.
